...
def ORBSLAM(vocab_path, settings_path, sequence_path):
    ...
    save_trajectory(slam.get_keyframe_points(), 'trajectory.txt')
    slam.shutdown()
...
def runSlam(vocab, calib, sequence):
    ORBSLAM(vocab, calib, sequence)


if __name__ == '__main__':
    if len(sys.argv) != 4:
        print('Usage: ./orbslam_mono_kitti path_to_vocabulary path_to_settings path_to_sequence')
    ORBSLAM(sys.argv[1], sys.argv[2], sys.argv[3])
